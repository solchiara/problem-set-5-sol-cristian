...
# Loop through each year and month, calling the scrape_enforcement_actions function
for year in years:
    for month_index, month in enumerate(months, start=1):  # Use index to pass correct month number
        scrape_enforcement_actions(year, month_index)

# Initialize an empty list to store DataFrames
df_final = []

# Read each CSV file generated by scrape_enforcement_actions and concatenate
for year in years:
    for month in months:
        file_name = f"enforcement_actions_{year}_{month}.csv"
...
# Concatenate all DataFrames in df_final list into a single DataFrame
...
# Write csv
df_final.to_csv("enforcement_actions_from_functions.csv", index=False)
...
topics = {
    ...
    "Bribery/Corruption": r"\bBribery|Corruption|Extortion|Embezzlement\b",  # Fixed regex
}

# Set the default topic to "Other"
df_enforcement['Topic'] = 'Other'

# Iterate over the dictionary to apply each topic condition
...
# Cleaning the district name of the enforcement actions
